...
class Categorias extends CI_Controller {
    ...
    public function core($categoria_id = NULL)
    {
        ...
        if(!$categoria_id) {
            //  Register / Cadastrar            
            $this->form_validation->set_rules('categoria_nome','Nome da categoria','trim|required|min_length[2]|max_length[40]|callback_valida_categoria_nome');
            ...
            if($this->form_validation->run()) {
                ...
                $data = array (
                    'titulo' => 'Cadastrar categoria filho',                    
    ...
    public function valida_categoria_nome($categoria_nome)
    {
        ...
        if(!$categoria_id) {
            ...
            if($this->core_model->get_by_id('categorias', array('categoria_nome' => $categoria_nome, 'categoria_id !=' => $categoria_id))) {
                $this->form_validation->set_message('valida_nome_categoria', 'A categoria já existe.');
                return FALSE;
            }else {
                return TRUE;
            }
    ...
    public function delete($categoria_id = NULL)
    {
        ...
        if(!$$categoria_id || !$this->core_model->get_by_id('categorias_pai', array('$categoria_id' => $$categoria_id))) {
            $this->session->set_flashdata('error', 'A categoria não foi encontrada');
            redirect('restrita/categorias');
        }
        
        if($this->core_model->get_by_id('categorias', array('$categoria_id' => $$categoria_id, '$categoria_ativa' => 1))) {
            $this->session->set_flashdata('error', 'Não é permitido excluir uma categoria ativa.');
            redirect('restrita/categorias');
        }   
